...
#####################################################################################################################################
    ...
    with c1:
        ...
        fig = go.Figure(data=[go.Table(
            ...
            cells=dict(values=[df_final.index, df_final.Var_fe_prct, df_final.FE, df_final.TR90, df_final.DMR],                       
                       ...
                       layout=go.Layout(height=850, width=480))
    ...
    with my_expander:        
        ...
        fig = go.Figure(layout=go.Layout(height=700, width=1000, plot_bgcolor='whitesmoke'))
        ...
        if nb_jours == 4:
            ...
            fig = go.Figure(data=[go.Table(
                ...
                cells=dict(values=[df_fe_tab.index, df1.J1, df1.J2, df1.J3, df1.J4, 
                           ...
                           layout=go.Layout(height=850, width=700))
        ...
        if nb_jours == 6:
            ...
            fig = go.Figure(data=[go.Table(
                ...
                cells=dict(values=[df_fe_tab.index, df1.J1, df1.J2, df1.J3, df1.J4, df1.J5, 
                           ...
                           layout=go.Layout(height=850, width=800))
        ...
        if nb_jours == 8:
            ...
            fig = go.Figure(data=[go.Table(
                ...
                cells=dict(values=[df_fe_tab.index, df1.J1, df1.J2, df1.J3, df1.J4, df1.J5, 
                           ...
                           layout=go.Layout(height=850, width=900))
...
#####################################################################################################################################   
    ...
    def dataframe_temp_dim(nbr_itv, flux, jsem, nb_jours, type_bts):
        ...
        df_temp['Nb AE'] = round((df_temp['med_ae'] * nbr_itv),1)           
        df_temp['ETP'] = round((df_temp['Nb AE'] * (df_temp['med_dmt']+15)) / 30 / 51,1)  
        
        df_temp['Diff. ETP'] = df_temp['ETP'] - df_temp['ETP'].shift(1)  
        df_temp['Diff. ETP'].fillna(0, inplace = True)
        
        tot_ae = df_temp['Nb AE'].sum()
        tot_etp = df_temp['ETP'].sum()/2/8    
                
        return df_temp[['Nb AE', 'ETP', 'Diff. ETP']], tot_ae, tot_etp
    ...
    df_concat = df_all
    df_concat.rename({'Nb AE':'All_AE', 'ETP':'All_ETP'}, axis = 1, inplace = True)
    df_concat = pd.concat([df_concat, df_tech[['Nb AE', 'ETP']]], axis = 1)
    df_concat.rename({'Nb AE':'TECH_AE', 'ETP':'TECH_ETP'}, axis = 1, inplace = True)
    df_concat = pd.concat([df_concat, df_cr[['Nb AE', 'ETP']]], axis = 1)
    df_concat.rename({'Nb AE':'CR_AE', 'ETP':'CR_ETP'}, axis = 1, inplace = True) 
